...
export function useLoadingTemplate() {
  ...
  useEffect(() => {
    ...
    if (replay?.duration) {
      const replayCap = replay.streaming ? 10000 : 5000;
    ...
    function updateMagic({
      ...
      if (
        ...
        if (
          ...
          if (paramSchema.enum) {
            const speed = (paramSchema.enum.length > 5 ? 1 : 2) * speedFactor;
    ...
    const requestUrl = new URL(selectedRecipe.path);
    ...
    const requestInfo: RecipeRequestInfo = {
