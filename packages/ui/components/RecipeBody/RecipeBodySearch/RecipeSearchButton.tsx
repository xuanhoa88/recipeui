...
export function RecipeSearchButton() {
  ...
  const _onSubmit = async () => {
    ...
    const fetchMethod = recipe.method;
    ...
    if (collectionModule) {
      ...
      for (const config of authConfigs) {
        ...
        const secretValue = secretRecord[secretKey];
    ...
    if (!collectionModule && recipe.authConfig) {
      const authConfigs: SingleAuthConfig[] =
    ...
    const clonedBody = structuredClone(fetchRequestBody);

    const clonedHeaders = structuredClone(fetchHeaders);
    const clonedUrl = new URL(path + url.search);
