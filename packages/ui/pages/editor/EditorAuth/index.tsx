...
import {
  MultipleAuthConfig as BaseMultipleAuthConfig,
  OAuth2AuthConfig,
  SingleAuthConfig as BaseSingleAuthConfig,
...
function SingleAuthConfig({
  ...
  editorAuthConfig: BaseSingleAuthConfig;
}) {
  if (
...
function BasicAuth({
  ...
  editorAuthConfig: Extract<BaseSingleAuthConfig, { type: RecipeAuthType.Basic }>;
}) {
  const currentSession = useRecipeSessionStore(
    (state) => state.currentSession
  )!;
...
function MultipleAuthConfig({
  ...
  editorAuthConfig: BaseMultipleAuthConfig;
}) {
  const currentSession = useRecipeSessionStore(
    (state) => state.currentSession
  )!;
